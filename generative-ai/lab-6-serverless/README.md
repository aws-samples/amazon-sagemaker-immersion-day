...
|---|---|
| [`01_data-preparation.ipynb`](./01_data-preparation.ipynb) | Streams `FreedomIntelligence/medical-o1-reasoning-SFT`, reformats into prompt/completion with `<think>` tags, splits 70/10/20, registers three datasets in the SageMaker AI Registry. |
| [`02_fine-tuning.ipynb`](./02_fine-tuning.ipynb) | Submits a serverless `SFTTrainer` job with `TrainingType.LORA`. Registers the result as a model package in your Model Registry. ~20 minutes. |
| [`03_deployment.ipynb`](./03_deployment.ipynb) | Deploys the merged fine-tuned model behind a SageMaker real-time endpoint with the DJL/LMI inference container. Closes with cleanup. |

The companion workshop pages live at
[`/generative-ai/lab-6-serverless/`](https://catalog.workshops.aws/) — see the workshop
...
The deployment notebook was renumbered from `04_deployment.ipynb` to
`03_deployment.ipynb` because the evaluation step (notebook 3 upstream) is
